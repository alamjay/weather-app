...
export const AirQualityCard: React.FC<props> = ({selectedLocation}: props) => {
  ...
  useEffect(() => {
    ...
    return (
      <div className="rounded-md shadow-lg h-[400px] w-[400px] bg-gray-200 py-8 flex flex-col items-center justify-between">
        <h2 className="font-serif text-3xl font-extrabold text-cyan-950">Air Quality</h2>
        <div className="flex flex-col gap-2 text-center">
          <h3 className="font-verdana font-black text-9xl text-green-600">{aqiData?.aqiDisplay}</h3>
          <p className="font-serif text-lg">AQI</p>
        </div>
        <h5 className="font-verdana text-xl font-semibold text-green-600">{aqiData?.category}</h5>
      </div>
    );
};